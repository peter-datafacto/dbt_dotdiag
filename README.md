...
dbt (data build tool) is a popular open-source framework for transforming raw warehouse data into production-ready data marts using version-controlled yaml configuration files and modular SQL. Beyond transformation, dbt can automatically generate a comprehensive documentation site using a project’s source code, metadata, and embedded documentation. This portal includes an interactive lineage graph that maps the entire project, and provides a GUI allowing users can pan, zoom, and filter to navigate complex model dependencies.
...
![close-up of a 20-model](example/diagrams/fig1_dbt_lin_graph_example.png)
 ...
 **Figure 3** was generated using **dbt_dotdiag**. 
 ```
 fig3
 ```
 **Figure 3: The dbt_dotdiag equivalent of Figure 2**
 
 Figure 3 maps the exact model subset shown in Figure 2. An objective comparison reveals that dbt_dotdiag provides several critical enhancements for formal documentation,
...
- **Core Target Models** (`--target_models`): Accepts one or more model names as a comma-separated string. This set forms the "anchor" of the diagram from which dependencies are traced.
    
- **Parent Depth** (`--parents`): Specifies the number of upstream dependency levels to include. A depth of `1` restricts the view to the immediate sources or parents of the target models.
    
- **Child Depth** (`--children`): Specifies the number of downstream dependency levels to include. A depth of `1` shows only the models immediately impacted by the target models.
    
- **Exclude Filter** (`--exclude`): Filters out specific models using a regular expression. For instance, providing `"rpt_.*"` would exclude all reporting layer models from the resulting diagram.
    
- **Manifest Path** (`--manifest_path`): The file path to a valid dbt `manifest.json`. While the file may be renamed, it must maintain the `.json` extension to be processed.
    
- **Output Path** (`--output_path`): The destination path for the generated diagrams. This path should include the **filename stem** only; the tool will automatically append the appropriate extensions (e.g., `.png`, `.svg`) based on the requested formats
    
- **Output Formats** (`--svg`, `--png`, `--jpg`, `--pdf`): Boolean flags that allow the user to generate the diagram in one or multiple formats simultaneously.
...
- **Visual Output:** The resulting publication-quality diagram generated by the tool.
    
- **Analysis:** A reflection on the visual results and how they inform the next stage of the investigation.

**Note on Data:** The project metadata used in this demonstration is derived from an anonymized and redacted production environment. To protect confidentiality, model names have been anonymized, and all business logic—including column names, SQL code, configurations, and internal identifiers—has been removed. The file used for this exercise, `manifest_anon.json` (included in this repository), contains only the structural model names and dependency mappings required to render the lineage.

## Step 1: The Big Picture

Imagine you are tasked with supporting a new marketing initiative to optimize email onboarding programs. After searching the dbt documentation portal for relevant keywords, you identify a few core reporting and fact models related to email performance. However, while you have found the "destination," the upstream logic remains a "black box" of interconnected dependencies that are difficult to parse in a global project view.
...
python dbt_dotdiag.py \
  ...
  --child_depth 0 \
  --pdf --svg \
  --output_path mkt1_full_upstream \
...
- **Child Depth (`--child_depth 0`):** Explicitly excludes downstream dependencies to focus solely on the "parent" logic. Since `--parent_depth` is omitted, the tool defaults to the full upstream tree.
...
- **Format Support (`--pdf`, `--svg`):** Generates both vector and document formats for versatile viewing and printing.
    

**Visual Output:**

**Figure 4: The Macro View**
...
python dbt_dotdiag.py \
  ...
  --parent_depth 1 \
  --pdf --svg \
  --output_path mkt1_direct_depends \
...
**Figure 5: Isolating Direct Dependencies**
...
**Figure 6a: Isolating Fact from Reporting (Fact)**

**Figure 6b: Isolating Fact from Reporting (Reporting)**
...
python dbt_dotdiag.py \
  ...
  --parent_depth 1 \
  --pdf --svg \
  --output_path mkt1_user_360_impact \
...
### Refinement: Targeted Exclusion and Metadata Branding
...
python dbt_dotdiag.py \
  ...
  --exclude ".*churn.*" \
  --pdf --svg \
  --output_path mkt1_user_360_impact_no_churn \
  --title "User 360 Dimensions (Excluding Churn)\n(File: mkt1_user_360_impact_no_churn)" \
  --show-info \
  --manifest_path manifest_anon.json
```
...
**Figure 8: Refinement: Targeted Exclusion and Metadata Branding**
